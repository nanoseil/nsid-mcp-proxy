...
| 12 | PARTIAL | Generated command pins `@nanoseil/nsid-mcp-auth@0.1.0-next.0`, not a dist-tag | Make package manifest version the single source, then assert the manifest-derived exact pin in Claude/Codex setup snapshots and document explicit upgrade/re-setup |
